...
public class ReadTif : MonoBehaviour
{
    ...
    internal class TIFF
    {
        ...
        public void Decode(string path)
        {
            ...
            Texture3D texture3D = new Texture3D(963, 305, 140, TextureFormat.R8, false);
            texture3D.SetPixelData(imageData.ToArray(), 0);
            texture3D.Apply();
            AssetDatabase.DeleteAsset("Assets/Textures/" + "neuron" + ".Asset");
            AssetDatabase.CreateAsset(texture3D, "Assets/Textures/" + "neuron" + ".Asset");
            AssetDatabase.SaveAssets();
            AssetDatabase.Refresh();
        }

        public void DecodePBD(string path)
        {
            sourceData = File.ReadAllBytes(path);
            byte[]  targetData;
            ...
            texture3D.Apply();
            string name = path.Substring(path.LastIndexOf('/')+1,8);
    ...
    void Start()
    {
        ...
        //byte[] byte1 = new byte[963 * 305 * 140];
        //Array.Copy(imgByte, fs.Length - 963 * 305 * 140, byte1, 0, 963 * 305 * 140);
        //Debug.Log(byte1.Length);
        //fs.Close();
        //Texture3D texture3D = new Texture3D(963,305,140 , TextureFormat.R8, false);
        //texture3D.SetPixelData(byte1, 0);
        //texture3D.Apply();
        //AssetDatabase.DeleteAsset("Assets/Textures/" + "neuron" + ".Asset");
        //AssetDatabase.CreateAsset(texture3D, "Assets/Textures/" + "neuron" + ".Asset");
        //AssetDatabase.SaveAssets();
