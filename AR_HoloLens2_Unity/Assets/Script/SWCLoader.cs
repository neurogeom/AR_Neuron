...
///
///</summary>
public class SWCLoader : MonoBehaviour
{
    ...
    public bool IsFromNetwork = false;
    public string swcContent;
...
//#endif
    ...
    private void ReadSWCFromNetwork(string swcContent)
    {
        swc_average = new Vector3(0, 0, 0);
        swc_list = new List<Vector3>();
        swc_map = new Dictionary<int, int>();
        string[] strs = swcContent.Split('\n');
        for(int i = 0; i < strs.Length; ++i)
        {
            if (strs[i].StartsWith("#")) continue;
            string[] words = strs[i].Split();
    ...
    */
    ...
    private void Awake()
    {
        swc_path = "E:\\163data_resource\\1\\02_1.v3dpbd_neutube_updated.swc";
        //swc_path = @"Assets\NeuronsPrefabs\tmp.swc";
        //swc_path = "1.swc";
        needScript = false;
        ...
        if (connection!=null)
        {
            //Debug.Log(connection.GetComponent<TestSocket>().swcContentList[0]);   
            swc_path= @"Assets\NeuronsPrefabs\tmp.swc";
        }
        ...
        //print(swc_list[10]);
        GameObject pObject = GameObject.Find("SWCParent");
        CreateNeuron();
        //CombineMesh(pObject);
        CreateBoundingBox(pObject);
        //CreateBoundingBox(GameObject.Find("SWCParent"));
        justShowSphere = 0;
        neuronFollowing = 0;
    }
}
